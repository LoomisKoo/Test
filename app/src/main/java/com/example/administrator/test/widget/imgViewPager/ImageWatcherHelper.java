...
public class ImageWatcherHelper {
    ...
    private ImageWatcherHelper(FragmentActivity activity) {
        holder = activity;
        activityDecorView = (ViewGroup) activity.getWindow()
                                                .getDecorView();
    }
    ...
    public void show(ImageView i, SparseArray<ImageView> imageGroupList, List<String> urlList,int position) {
        init();
        final boolean displaySuccess = mImageWatcher.show(i, imageGroupList, urlList,position);
        if (displaySuccess) {
            appendOtherView();
        }
    }

    public void show(int initPosition, SparseArray<ImageView> imageGroupList, List<String> urlList) {
        init();
    ...
    private void init() {
        ...
        if (indexProvider != null) {
            mImageWatcher.setIndexProvider(indexProvider);
        }
        if (loadingUIProvider != null) {
            mImageWatcher.setLoadingUIProvider(loadingUIProvider);
        }
    ...
    private void appendOtherView() {
        if (otherView != null) {
            if (otherView.getId() == View.NO_ID) {
                otherView.setId(VIEW_DECORATION_ID);
            }
