...
public class ImageWatcher extends FrameLayout implements GestureDetector.OnGestureListener, ViewPager.OnPageChangeListener {
    ...
    public boolean show(List<String> urlList, int initPos) {
        if (urlList == null) {
     ...
     */
    public boolean show(ImageView i, SparseArray<ImageView> imageGroupList, final List<String> urlList) {
     ...
     * @param urlList        被加载的图片url列表，数量必须大于等于 imageGroupList.size。 且顺序应当和imageGroupList保持一致
     */
    public boolean show(ImageView i, SparseArray<ImageView> imageGroupList, final List<String> urlList, int position) {
        if (i == null || imageGroupList == null || urlList == null) {
            Log.e("ImageWatcher", "i[" + i + "]  imageGroupList[" + imageGroupList + "]  urlList[" + urlList + "]");
            return false;
        }
    ...
    public boolean show(int position, SparseArray<ImageView> imageGroupList, final List<String> urlList) {
    ...
    private void showInternal(ImageView imageView, SparseArray<ImageView> imageGroupList, final List<String> urlList) {
        if (loader == null) {
    ...
    public void notifyItemChanged(int position, String newUri) {
        if (mUrlList == null || mUrlList.size() <= position || position < 0) {
            return;
        }
    ...
    public boolean onFling(MotionEvent e1, MotionEvent e2, float velocityX, float velocityY) {
        if (iSource != null && mTouchMode != TOUCH_MODE_AUTO_FLING && mPagerPositionOffsetPixels == 0) {
            ...
            if (velocityX > 0 && vsCurrent.translationX == (vsDefault.width * (vsCurrent.scaleX - 1) / 2) && "horizontal".equals(imageOrientation)) {
                ...
                animFling(iSource, ViewState.write(iSource, ViewState.STATE_TEMP)
                                            .translationX(endTranslateX)
                                            .translationY(endTranslateY), (long) (1000000 / maxVelocity));
                return true;
            }
        }
        return false;
    ...
    private void handleDragGesture(MotionEvent e2, MotionEvent e1) {
        ...
        if (vsDefault == null || vsTouchDrag == null) {
            return;
        }
        final float moveY           = e2.getY() - e1.getY();
        final float moveX           = e2.getX() - e1.getX();
    ...
    private void handleScaleGesture(MotionEvent e2) {
        ...
        if (vsDefault == null || vsTouchScale == null) {
            return;
        }
...
//        float distance = (float) Math.sqrt(deltaX * deltaX + deltaY * deltaY);
    ...
    private void handleDoubleTapTouchResult() {
        ...
        if (vsCurrent.scaleY <= vsDefault.scaleY && vsCurrent.scaleX <= vsDefault.scaleX) {
            ...
            animSourceViewStateTransform(iSource,
                                         ViewState.write(iSource, ViewState.STATE_TEMP)
    ...
    private void handleScaleTouchResult() {
        ...
        ViewState vsTemp = ViewState.copy(vsDefault, ViewState.STATE_TEMP)
                                    .scaleX(endScaleX)
                                    .scaleY(endScaleY);
    ...
    private void handleExitTouchResult() {
        ...
        if (mExitRef > 0.75f) {
            ...
            if (vsOrigin != null) {
                if (vsOrigin.alpha == 0) {
                    vsOrigin.translationX(iSource.getTranslationX())
                            .translationY(iSource.getTranslationY());
                }
                ...
                loader.load(imageView.getContext(), mUrlList.get(position), new LoadCallback() {
                    ...
                    public void onResourceReady(Drawable resource) {
                        ...
                        ViewState vsDefault = ViewState.write(imageView, ViewState.STATE_DEFAULT)
                                                       ...
                                                       .translationX(sourceDefaultTranslateX)
                                                       .translationY(sourceDefaultTranslateY);
        ...
        private boolean setDefaultDisplayConfigs(final ImageView imageView, final int pos, boolean hasPlayBeginAnimation) {
            ...
            if (originRef != null) {
                ...
                ViewState.write(imageView, ViewState.STATE_ORIGIN)
                         .width(originRef.getWidth())
                         .height(originRef.getHeight());

                final Drawable bmpMirror = originRef.getDrawable();
                if (bmpMirror != null) {
                    int bmpMirrorWidth = bmpMirror.getBounds()
                                                  .width();
                    int bmpMirrorHeight = bmpMirror.getBounds()
                                                   .height();
                    ViewState vsThumb = ViewState.write(imageView, ViewState.STATE_THUMB)
                                                 .width(bmpMirrorWidth)
                                                 .height(bmpMirrorHeight)
                                                 .translationX((mWidth - bmpMirrorWidth) / 2)
    ...
    private void animFling(ImageView view, final ViewState vsResult, long duration) {
        ...
        animFling = ViewState.restoreByAnim(view, vsResult.mTag)
